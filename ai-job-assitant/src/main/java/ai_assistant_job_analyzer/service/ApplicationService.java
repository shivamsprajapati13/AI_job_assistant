...
import ai_assistant_job_analyzer.dto.ApplicationHistoryDto;
import ai_assistant_job_analyzer.dto.SendApplicationRequest;
import ai_assistant_job_analyzer.entity.JobApplication;
import ai_assistant_job_analyzer.repository.JobApplicationRepository;
...
public class ApplicationService {
    ...
    public JobApplication saveDraft(String userEmail, SendApplicationRequest request) {
        JobApplication app = JobApplication.builder()
                ...
                .status(JobApplication.ApplicationStatus.DRAFT)
                .build();
    ...
    public JobApplication updateDraft(Long id, String userEmail, SendApplicationRequest request) {
        ...
        if (app.getStatus() == JobApplication.ApplicationStatus.SENT) {
        ...
        app.setStatus(JobApplication.ApplicationStatus.DRAFT);
        return repository.save(app);
    }
    ...
    private JobApplication sendExistingApplication(Authentication auth, String userEmail,
        ...
        try {
            ...
            app.setStatus(JobApplication.ApplicationStatus.SENT);
            app.setSentAt(LocalDateTime.now());
        } catch (Exception e) {
            log.error("Failed to send application", e);
            app.setStatus(JobApplication.ApplicationStatus.FAILED);
